...
pub fn find_recent_files() -> Vec<(PathBuf, u64)> {
    ...
    // Parcours parallèle des dossiers pertinents
    ...
    for path in paths {
        if let Some(modified) = get_modified_time(&path) {
            ...
            if heap.len() > 75 {
                heap.pop(); // Garde les 50 plus récents
            }
        }
    }
    ...
    heap.into_sorted_vec()
        .into_iter()
        .map(|Reverse((modified, path))| (path, modified)) // Inverse l'ordre ici
        .collect()
}
...
pub fn find_file_btree(filename: &str, tree: &BTreeMap<String, String>) -> bool {
    tree.contains_key(filename)
}
